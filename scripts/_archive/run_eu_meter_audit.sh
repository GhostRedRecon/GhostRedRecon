...
CHANNELS=(${CHANNELS_OVERRIDE:-868.30 868.95 869.525})
STAMP="$(date -u +%Y%m%dT%H%M%SZ)"
RAW_REPORT="$REPORT_DIR/eu_meter_audit_${STAMP}.json"
SUMMARY_REPORT="$REPORT_DIR/eu_meter_audit_${STAMP}.txt"
TRACE_LOG="$REPORT_DIR/eu_meter_audit_${STAMP}.log"
TMP_DIR="$(mktemp -d)"
...
main() {
  ...
  log "capturing EU meter / W-MBus sweep"
  local first=1
  local idx=0
  ...
  log "running EU meter validation"
  local validator_args=()
  for idx in $(seq 1 ${#CHANNELS[@]}); do
    validator_args+=("--snapshot" "$TMP_DIR/ch${idx}.json")
  done
  python3 "$ROOT_DIR/scripts/validate_eu_meter_detection.py" "${validator_args[@]}" > "$TMP_DIR/validation.json"
...
metrics = validation.get("eu_meter_validation") or {}
lines = [
    "GhostRedRecon EU Meter / Wireless M-Bus Audit Report",
    f"Base URL: {base_url}",
...
for label, payload in snapshots.items():
    ...
    meter_signals = sum(
        ...
        if any(token in str(signal).lower() for token in ("meter", "wireless m-bus", "wmbus", "utility"))
    )
    lines.append(f"{label}:")
    lines.append(f"  signals={len(signals)} meter_signals={meter_signals} devices={len(devices)}")
    if devices:
        top = devices[0]
        lines.append(
            "  top_device="
            f"{top.get('device_id')} protocols={top.get('protocols')} "
            f"device_type={top.get('device_type')} product={top.get('product')}"
        )
    else:
...
for key in [
    ...
    "meter_signal_ratio",
    "meter_device_ratio",
    "wireless_mbus_device_ratio",
    "typed_meter_device_ratio",
    "consistency_score",
]:
