...
RAW_REPORT="$REPORT_DIR/subghz_audit_${STAMP}.json"
SUMMARY_REPORT="$REPORT_DIR/subghz_audit_${STAMP}.txt"
TRACE_LOG="$REPORT_DIR/subghz_audit_${STAMP}.log"
TMP_DIR="$(mktemp -d)"
...
on_error() {
  ...
  log "  SKIP_STACK_BOOTSTRAP=1 DWELL_SECONDS=${DWELL_SECONDS} LIMIT=${LIMIT} bash $ROOT_DIR/scripts/run_subghz_audit.sh ${BASE_URL}"
  exit "$exit_code"
}
trap on_error ERR

log() {
  printf '[subghz-audit] %s\n' "$*" | tee -a "$TRACE_LOG"
}
...
main() {
  ...
  log "capturing Sub-GHz channel cycle"
  local first=1
  local idx=0
  ...
  log "running LoRa/Sub-GHz consistency validation"
  local validator_args=()
  for idx in $(seq 1 ${#CHANNELS[@]}); do
    validator_args+=("--snapshot" "$TMP_DIR/ch${idx}.json")
  done
  python3 "$ROOT_DIR/scripts/validate_lora_detection.py" --base-url "$BASE_URL" --limit 250 "${validator_args[@]}" > "$TMP_DIR/validation.json"
...
lines = []
lines.append("GhostRedRecon Sub-GHz Audit Report")
...
for label, payload in snapshots.items():
    ...
    lines.append(f"  signals={len(signals)} lora_signals={lora_signals} recurring_hits={recurring_hits} devices={len(devices)} entities={len(entities)}")
    if signals:
        first = signals[0]
        lines.append(
            "  top_signal="
...
lora_validation = validation.get("lora_validation") or {}
